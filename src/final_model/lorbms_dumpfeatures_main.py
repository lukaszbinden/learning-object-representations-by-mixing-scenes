...
from lorbms_dumpfeatures_model import DCGAN
...
def main(argv):
    ...
    print('main <-- [%s s, epochs: %d, extractor_type: %s]' % (str(params.duration), params.epochs, params.extractor_type))
...
def init_main(argv):
    ...
    # if not file:
    file = []
    file.append('params_dumpfeatures.json')
    file = file[0]
    params = Params(file)
    plausibilize(params)
