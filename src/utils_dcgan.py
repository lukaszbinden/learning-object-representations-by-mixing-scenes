...
def save_images_7cols(imgs1, imgs2, imgs3, imgs4, imgs5, imgs6, imgs7, grid_size, batch_size, image_path, invert=True, channels=3, maxImg=None, addSpacing=None):
    if invert:
...
def imsave_7cols(imgs1, imgs2, imgs3, imgs4, imgs5, imgs6, imgs7, grid_size, batch_size, path, channels, maxImg=None, addSpacing=None):
    ...
    img = np.ones((h * int(grid_size[0]), w * int(grid_size[1]) + 4 + (spacing * 5), channels))  # (spacing * 5) -> assuming 7 images are given
    ...
    for i in range(grid_size[0]):
        ...
        if imgs7 is not None:
            j = 6
            img[i * h:(i + 1) * h, j * w + spacing * 6:(j + 1) * w + spacing * 6, :] = imgs7
