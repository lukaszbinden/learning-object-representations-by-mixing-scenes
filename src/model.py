...
class DCGAN(object):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('generator') as scope_generator:
            ...
            # each chunk is initialized with 1's (64,32)
            ...
            for tile_id in range(0, NUM_TILES): # for each tile feature slot
                f_1_tile_feature = self.f_x1_composite[:, tile_id * self.feature_size:(tile_id + 1) * self.feature_size]
                assert f_1_tile_feature.shape[1] == self.feature_size
                f_2_tile_feature = self.f_x2_composite[:, tile_id * self.feature_size:(tile_id + 1) * self.feature_size]
                assert f_2_tile_feature.shape[1] == self.feature_size
                f_feature_selected = tf.where(tf.equal(mask[tile_id], 1), f_1_tile_feature, f_2_tile_feature)
