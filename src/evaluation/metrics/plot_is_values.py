...
def main():
    ...
    metrics = pd.DataFrame(columns=['training_epoch', 'test_fid', 'test_is_sm', 'test_is', 'train_fid', 'train_is'])
    test_iss = pd.DataFrame(columns=['training_epoch', 'test_is'])
    ...
    for index, js in enumerate(test_json_files):
        with open(os.path.join(metrics_test_path, js)) as json_file:
            ...
            test_iss.loc[i] = [model_iteration, is_mean]
    ...
    # print(test_iss)
        test_iss = test_iss.sort_values(by=['training_epoch'], ascending=True)
    # print(test_fids['test_fid'].tolist())
    # test_fid_sm = smooth(test_fids['test_fid'], 19)
    test_is_list = test_iss['test_is'].tolist()
    test_is_sm = smooth_tb(test_is_list, 0.9)
    ...
    for key in iter_to_values.keys():
        ...
        if iter_to_values[key]['train_fid']:
            metrics.loc[key] = [iter_to_values[key]['training_epoch'], iter_to_values[key]['test_fid'], test_is_sm[id], iter_to_values[key]['test_is'], iter_to_values[key]['train_fid'], iter_to_values[key]['train_is']]
        else:
    ...
    mplot = metrics.plot.line(x='training_epoch', y='test_is', ax=ax, color='b')
    metrics.plot.line(x='training_epoch', y='train_is', ax=ax, color='orange')
    metrics.plot.line(x='training_epoch', y='test_is_sm', color="r", ax=ax)
    ...
    print("IS max: ", max(test_is_list))
    print("IS mean: ", np.mean(test_is_list))
    print("IS stddev: ", np.std(test_is_list))
