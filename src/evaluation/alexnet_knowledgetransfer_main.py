...
from alexnet_knowledgetransfer_model import DCGAN
...
def main(argv):
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
        dcgan = DCGAN(sess, params=params, batch_size=params.batch_size, epochs=params.epochs, \
                       image_shape=[params.image_size, params.image_size, 3], is_train=params.is_train)
...
def init_main(argv):
    ...
    if not file:
        file.append('params_knowledgetransfer.json')
    file = file[0]
    params = Params(file)
    plausibilize(params)
...
def create_dirs(argv, params):
    ...
    if params.is_train:
        ...
        params.dataset_path = params.train_tfrecords_path

    else: # test
        params.dataset_path = params.dataset_path_val
        assert 1 == 0, "TEST-Fall noch genau ausarbeiten!"
