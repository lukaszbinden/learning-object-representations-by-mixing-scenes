...
class DCGAN(object):
    ...
    def build_model(self):
        ...
        y_onehot = tf.one_hot(labels, self.params.number_of_classes, dtype=tf.int32)
        y_onehot = tf.reshape(y_onehot, [self.batch_size, self.params.number_of_classes])
        ########################### Dataset END
    ...
    def train(self, params):
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                if step % 1000 == 0:
                    lbls = self.sess.run(self.labels_onehot)
                    print("labels onehot [%d]: %s" % (step, str(lbls[0].tolist())))
