...
ImageNet encoder (AlexNet) & 51.6\% & $\pm$0.2  \\ % 29.04: pretraining: log_alexnet_No2_60ep.out (224px!), eval: log_linearcls_stl10_alexnet_after_pretrain_No1.out (64px!)
...
\begin{appendices}
\section{Appendix}
\subsection{Autoencoder calibration}\label{app:ae_calibration}
As an intermediate step in the design of the neural architecture for our method, we conduct experiments to calibrate the autoencoder to improve faithfulness and realism of the reconstruction of the input image. To that end, we take the autoencoder (i.e. generator) and the discriminator without the classifier and train it on the L1 reconstruction and the GAN loss. As shown in Figure~\ref{fig:ae_calib_graphs}, the autoencoder with resize convolution in the decoder (blue curve) outperforms all the other experiments that use transposed convolution.
\begin{figure}[h]
...
\end{subfigure}
\caption{PSNR and L1 reconstruction loss vs. training step. The autoencoder with resize convolution in the decoder (blue curve) achieves the best values.}
