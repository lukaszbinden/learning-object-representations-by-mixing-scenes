...
\par \textbf{Discriminator loss function} the discriminator needs the cross entropy loss function above since it has a very specific function (to discriminate among two classes) and the cross entropy is the “best” way of doing this. cf. https://danieltakeshi.github.io/2017/03/05/understanding-generative-adversarial-networks/

\par \textbf{Generator loss function} 
...
The encoder and decoder together as the generator $G$ receive a composite loss that is a weighted sum of three losses: the reconstruction loss, the standard GAN loss and the classifier loss. The reconstruction loss imposes a constraint on the autoencoder that steers it to produce images to closely match the originals $\hat{I}_{ref} \approx I_{ref}$ and is given by
\begin{equation} \label{eq:4}
    L_{rec} = \mathbb{E}_{I_{ref}\sim p_{data} (I_{ref})}\big[ \Vert I_{ref} - \hat{I}_{ref} \Vert_p + \Vert I_{ref} - I'_{ref} \Vert_p \big]
\end{equation}
where $p = 1$ or $p = 2$. 

However, the deficiency with the use of $l_1$ or $l_2$ (i.e. mean squared error) as a loss function is blurry predictions that $D$ can easily discriminate. As in [53], to counteract this flaw we introduce an adversarial loss, the standard GAN loss, to guide $G$ to produce sharp predictions as follows:
...
Thirdly, to drive the disentanglement in the latent space representation of the images, we impose the classifier constraint as follows:
\par TODO 19.1: (3) CLS loss

To combine the three losses we introduce coefficient parameters $\lambda_{rec}$, $\lambda_{adv}$ and $\lambda_{CLS}$. By means of these we can optimize the tradeoff between similarity with the original images (\ref{eq:4}), sharp predictions (2) and disentanglement of the four objects (3). Thus we get the final loss on $G$ as follows:
...
Here we calculate for the generated test images Inception Scores (IS) [99] (see Table 1) and FID [98].
