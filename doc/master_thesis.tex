...
\subsection{Concept}
Our main objective is the following: We want to build an unsupervised system that learns object representations by mixing objects from different scenes. To this end, the system identifies and extracts objects in images and represents them in a lower dimensional latent space in a disentangled and invariant state. By editing the latent space representation we can move objects around in a non-linear way. Eventually the mixed representation is transformed back to image space and thereby creates a new scene. For example, the system could extract an aircraft object from an airport, extract a lawn mower object in a public park, swap the two objects in latent space and create two new scenes with a lawn mower in an airport and an aircraft in a public park. 
The system should therefore enable us to move objects around or swap them independently of their background and merge them into a new scene. We want to transfer objects between images simply by swapping the respective feature chunks. Ideally, the system should be able to cut-and-paste objects between different scenes where an object and its attributes such as size or surrounding background are adapted in a non-linear way. The model is built on the assumption that each of the 4 quadrants of an image contains at most one object as exemplified in Figure~\ref{fig:livingroom_4quad}. 
...
\begin{enumerate}
  \item many prior works have shown that disentanglement works with aligned data. Thus spatial location matters for disentangling factors of variation. In our system the data is non-aligned and spatial location of objects highly stochastic as we use a natural dataset.
  \item  in most datasets used by other methods the objects have little variation in spatial size. Our system however faces natural data where objects take on any size and any spatial location.
  \item an extra source of difficulty in extracting and moving an object is its impact on the proximity pixels of its position such as shading or partly occluded other objects.
\end{enumerate}

% TODO 04.03 somehow somewhere talk about GENERALIZATION
Essentially, the unsupervised model tries to discover structure in the dataset to learn its manifold and interpolate through it. However, we expect this a very difficult task as the model faces a natural dataset. The latent representation is learned as a concatenation of 4 feature chunks where each chunk exposes one object from the input and represents a disentangled factor of variation, respectively. Whereas \cite{DisentFacOfVarByMixTh} considers an \textit{image attribute} as a factor of variation in single-object input data, our model views an \textit{image object} as a factor of variation in multi-object input data~\footnote{the disentangling of image attributes within disentangled objects provides an interesting avenue for future work.}. The latent representation can also be considered as a \textit{compositional representation}~\cite{SpatialBDecoder} consisting of components that can be recombined. % ", and such recombination underlies generalization"
Our model architecture is designed to encourage such representations. See Section~\ref{subsec:model_arch} for more details. The model uses a GAN to increase the realism of the generated scenes. The GAN generator is composed of an autoencoder where the encoder computes the low-dimensional latent representation given an input image and the decoder generates the output image. The proposed method is a conditional generative model where both the generator and the discriminator are conditioned on the input dataset. 

TODO at work 
% About LORBMS.txt
% the intent of the system, the basic motivation for all of this is that we would like to be able to move objects around by editing the feature space
% THEN WE GET A "MIXED SCENE" WHERE WE COMBINE OBJECTS FROM I1 AND OBJECTS FROM I2
% SO WHAT THIS WILL DO IS IT WILL LEARN A MODEL THAT IDENTIFIES OBJECTS IN THE SCENE AND LEARNS TO COMPOSE THEM SO YOU CAN MERGE SCENES
% - IT WILL BE SPACIAL object-to-object translation
% - we mix things only when things are sufficiently similar -> see algorithm
% - aim: only mix things that are compatible/similar -> should see pairs of images with meaningful mixes 
% - so we might expect at least to swap two different people, or two different trees, cars with maybe different poses and then we see a rendered image with that change
% - the purpose in the end is to build a representation where I take a feature from one and plug it there and then the rendering adapts the content - that's the ultimate goal
% - in the end we want to be able to move objects around so that if we go into the feature space and move something we should see that object move in the output picture
% -> at the moment we're discussing very coarse movements (e.g. just two tiles)
% - we try to mix only images that have some similarity - we would swap content that would look different but we're swapping similar content at the same location (would happen only in same position of the image) -> using L2 on the feature 
% -> we are not segmenting but moving things around while making sure the result is still realistic
% - perhabs you are copying a parked car to that street, the car may be not in the right "intuition"
% - as you copy it onto that street hopefully it will put it in the right "intution" according to the street
% - OR YOU COPY A PERSON IN A SCENE WHERE THE PERSON IS FURTHER AWAY, BUT IN THE TARGET IMAGE THE SCALE OF THE PERSON HAS TO BE SMALLER
% - i suspect that those features will be supergood representations of the objects -> because they should be somehow invariant or have some very clear separation between the description of the object and its pose, scale etc
% - SO ANOTHER THING THAT ONE WOULD DO AFTER DOING THIS IS TAKE THE ENCODER AND SEE HOW WELL THAT GUY TRANSFERS TO OTHER TASKS (possible continuation)
% - that we have an architecture where we can take images and mix the content and we can be selective, so we can have a way to say what we can mix,  as long as what we mix it will then show the mixture of these two images
...
\par add short discussion L2 vs L1 
(cf http://www.chioka.in/differences-between-l1-and-l2-as-loss-function-and-regularization/)
