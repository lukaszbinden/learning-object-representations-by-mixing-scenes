...
\caption{The 4 object assumption: 0 or 1 object per quadrant.} % An image as quadrants with one or zero objects each. An image split into quadrants.
...
TODO TBD see disentangling paper eq (4)
...
\includegraphics[scale=0.49]{images/appendix/FID_test_vs_train_v1.png}
\caption{FID of generated images (lower is better).}
\label{fig:fid_test_train}
\end{subfigure}
\begin{subfigure}{0.495\textwidth}
\includegraphics[scale=0.49]{images/appendix/IS_test_vs_train_v1.png}
...
See Appendix~\ref{app:samples} for more rendering examples.
...
\hline
Random & 10.0\% & - \\
Random encoder & 43.4\% & $\pm$0.4 \\ % 02.05: log_linearcls_random_encoder_No2_96px.out
Random encoder (finetuned) & ....\% & $\pm$... \\
PASCAL encoder & 47.1\% & $\pm$0.2  \\ % 02.05 (96px) pretraining: log_pascalvoc_6th_NoResizeTo64Use224.out; eval: log_linearcls_pascal_encoder_No3_6th_96px.out
% COULD PASCAL encoder (AlexNet) & ...\% & $\pm$...  \\
ImageNet encoder (AlexNet) & 62.4\% & $\pm$0.3  \\ % 02.05: pretraining: log_alexnet_No2_60ep.out (224px!), eval: log_linearcls_imagenet_alexnet_after_pretrain_No2_96px.out
STL-10 encoder & 78.7\% & $\pm$0.1 \\ % 02.05 (64px: 77.2): 02.05: pretraining: log_stl-10-pretraining__encoder_No3_96px.out; eval: log_linearcls_stl10_encoder_after_pretrain_No2_96px.out
STL-10 encoder (AlexNet, conv1-conv5) & 60.9\% & $\pm$0.1 \\ % 02.05: pretraining: log_stl-10-pretraining__alexnet_No2_96px.out; eval: log_linearcls_stl10_alexnet_after_pretrain_No3_96px.out
\hline
Swersky \textit{et al.}~\cite{Stl10TlExp2Comp} & 70.1\% & $\pm$0.6 \\
Jenni \& Favaro~\cite{SpotArtifacts} (frozen) & 76.9\% & $\pm$0.1 \\
\hline
(Ours) LORBMS encoder & 36.5\%  & $\pm$0.2  \\ % 03.05: log_linearcls_lorbms_encoder_ep73_frozen_96px_No1.out
(Ours) LORBMS encoder (finetuned) & TBD\% & $\pm$1.0  \\ % 03.05: 
(Ours) LORBMS discriminator & TBD\%  & $\pm$TBD  \\
...
\par Perhaps: think about the size of k in the clustering algo and its implication for the scene mixing algo

\par paper [66]: consider applying random jitter to dataset?

\par increase resolution to 128x128?
...
% TODO: mention Spatial broadcast decoder
\begin{enumerate}
...
\subsection{Sheets of mixed scene renderings}\label{app:samples}
Following up on Section~\ref{sec:scene_renderings}, we show more test examples of mixed scene renderings using our model in Figures~\ref{fig:results_lorbms_mixed_scenes_appendix_1} and \ref{fig:results_lorbms_mixed_scenes_appendix_2}.
\input{includes/results_lorbms_mixed_scenes_appendix.tex}
