...
The encoder and decoder together receive a composite loss that is a weighted sum of three losses: (1) the reconstruction loss, (2) the standard GAN loss and (3) the classifier loss. The reconstruction loss imposes a constraint on the autoencoder that steers it to produce images to closely match the originals $\hat{I}_{ref} \approx I_{ref}$ and is given by
\begin{equation} \label{eq:4}
    L_{rec} = \mathbb{E}_{I_{j}\sim p_{data} (I_{j})}\big[ \Vert I_{ref} - \hat{I}_{ref} \Vert_2 + \Vert I_{ref} - I'_{ref} \Vert_2 \big]
\end{equation}
where $j \in \{ref,q1,q2,q3,q4\}$. 
\par TODO 10.1: (2) GAN loss und (3) CLS loss
Including the coefficients, the composite loss function for the encoder and autoencoder becomes:
\begin{equation} \label{eq:g_loss_comp}
    L_{G} = \lambda_rec L_{rec} + \lambda_{GAN} L_{GAN} + \lambda_{CLS} L_{CLS}
\end{equation}
